...
def compute_hog(roi, hog, config):
    ...
    cv.imshow("roi", cv.resize(roi, (-1, -1), fx=5, fy=5))
    cv.waitKey(1)
...
# main
print("training HOG&SVM based classifier started: %s" % params.get_current_time())

classification_dataset = params.ClassificationDataset()
hog_svm_config = params.HogSvmClassifier()
...
samples, labels = [], []
label, type_counter, piece_counter = 0, 0, 0
for p_type in sorted(os.listdir(classification_dataset.train)):
    print("\n > loading '%s' pieces ..." % p_type)
    type_folder = os.path.join(classification_dataset.train, p_type)
    for p_name in sorted(os.listdir(type_folder)):
        ...
        label = 100 * type_counter + piece_counter
...
svm.save("svm_classifier_py.yaml")

print("\ntraining HOG&SVM based classifier ended:   %s" % params.get_current_time())
